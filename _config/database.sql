...
CREATE TABLE invitation_room (
    ...
    id_room INT,
    statut VARCHAR(255),
    FOREIGN KEY (id_user_invited) REFERENCES user(id_user),
    FOREIGN KEY (id_room) REFERENCES room(id_room)
);
...
CREATE TABLE blocage (
    ...
    FOREIGN KEY (id_user_bloqueur) REFERENCES user(id_user),
    FOREIGN KEY (id_user_bloque) REFERENCES user(id_user)
);